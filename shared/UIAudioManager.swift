...
class UIAudioManager: ObservableObject {
    ...
    @Published var frequencyRange: Double = 1.0
    ...
    @Published var visualFrequency: Double = 440.0
    @Published var visualAmplitude: Double = 0.0
        ...
        let sourceNode = AVAudioSourceNode { [weak self] _, _, frameCount, audioBufferList -> OSStatus in
            ...
            for buffer in pointer {
                ...
                for frame in 0..<Int(frameCount) {
                    let value = sin(phase) * volume
                    let floatValue = Float32(value)
                  
                    for channel in 0..<channelCount {
                        frames?[frame * channelCount + channel] = floatValue
                    }
                  
                    // 更新波形数据用于可视化
                    if frame % 10 == 0 { // 降低采样频率
                        DispatchQueue.main.async {
                            self.waveformData[self.waveformIndex] = floatValue
                            self.waveformIndex = (self.waveformIndex + 1) % self.waveformData.count
                            self.visualAmplitude = Double(abs(floatValue))
                        }
                    }
    ...
    func updateFrequency(_ frequency: Double) {
        ...
        DispatchQueue.main.async {
            self.visualFrequency = mappedFrequency
        }
    }
    ...
    // 获取音符名称
    func getNoteName(frequency: Double) -> String {
        ...
        let h = 12 * log2(frequency / c0)
        let octave = Int(h / 12)
        let noteIndex = Int(h.truncatingRemainder(dividingBy: 12))
